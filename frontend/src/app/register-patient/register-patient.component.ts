...
export class RegisterPatientComponent implements OnInit {
    ...
    registerPatient() {
        ...
        this.servicePatient.readByUsername(this.username).subscribe((patient: Patient) => {
            if (patient != null) {
                this.exist = true
            }
            if (this.exist) {
        ...
        this.servicePatient.readByMail(this.mail).subscribe((patient: Patient) => {
            if (patient != null) {
                this.exist = true
            }
            if (this.exist) {
        ...
        this.serviceSession.registerPatient(this.firstname, this.lastname, this.username, this.password, this.address, this.phone, this.mail, this.image).subscribe(respObj => {
            if (respObj['message'] == 'ok') {
                this.router.navigate(["patient"])
                // check is this.image is base64
            } else {
                this.message = "Error!"
                this.alert.style.visibility = "visible"
            }
