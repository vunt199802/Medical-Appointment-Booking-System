...
export class ManagerRegisterDoctorComponent implements OnInit {
  ...
  constructor(private serviceManager: ManagerService, private serviceSpecialization: SpecializationService) {
  }
  ...
  onSelectFile(event) {
    if (event.target.files) {
      ...
      // picture limit pixel 100x100 - 300x300
  ...
  registerDoctor() {
    ...
    {
      this.serviceManager.checkDoctorUsername(this.username).subscribe((doctor: Doctor) => {
    ...
    this.serviceManager.checkDoctorMail(this.mail).subscribe((doctor: Doctor) => {
      if (doctor != null) {
        this.exist = true
      }
    })
    ...
    this.serviceManager.registerDoctor(this.firstname, this.lastname, this.username, this.password, this.address, this.phone, this.mail, this.licenceId, this.specialization, this.medicineBranch, this.image).subscribe(respObj => {
